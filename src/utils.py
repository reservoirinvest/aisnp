...
def classify_pf(pf):
    ...
    pf.loc[(pf.state == 'tbd') & (pf.secType == 'OPT') & (pf.position > 0), 'state'] = 'orphaned'

    # Update status field for sowed options    
    pf.loc[(pf.state == 'tbd') & (pf.secType == 'OPT') & (pf.position < 0), 'state'] = 'sowed'
...
def classify_open_orders(df_openords, pf):
    ...
    reaping_mask = opt_orders.apply(
        lambda row: (
            row.action == 'BUY' and 
            not pf[(pf.secType == 'OPT') & 
                   (pf.symbol == row.symbol) & 
                   (pf.right == row.right) & 
                   (pf.strike == row.strike)].empty
        ), 
        axis=1
    )
    ...
    straddle_symbols = opt_orders[
        (opt_orders.action == 'BUY')
    ].groupby('symbol').filter(lambda x: len(x) >= 2)['symbol'].unique()
    ...
    deorphaning_mask = opt_orders.apply(
        lambda row: (
            ...
            row.symbol not in pf[(pf.secType == 'STK')].symbol and 
